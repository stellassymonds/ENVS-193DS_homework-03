...
ggplot(data = kelp, # starting data frame
  ...
  geom_qq() 
```
I checked the assumption of normality by creating a QQ plot. No curved pattern in the data is evident on the plot, meaning the data is approximately normally distributed. 
...
e. Lower temperatures are conducive for higher giant kelp growth rates. Keeping temperatures below a certain value may be beneficial for giant kelp growth and giant kelp health by association. 

f. ### Run the other test
```{r}
...
We found a moderate negative relationship between temperature and giant kelp frond elongation rate (Spearman ρ = -0.69, S = 9216, p < 0.001, $\alpha$ = 0.05). The Spearman rank correlation and Pearson's correlation tests would have led me to make the same decision to reject the null hypothesis and conclude that the relationship between temperature and giant kelp frond elongation rate is moderately negative. Both p values were less than 0.001, representing the reasoning for rejection of the null, and produced a correlation coefficient of about -0.69, representing the strength of the relationship. 
...
a. ![Kruskal-Wallis Figure](kruskal.png)

The authors use Kruskal-Wallis tests to address whether increased contact between calves and the herd, horned and without horns, has an association with the calves' growth and health.

b. The authors clearly represented their statistics in figures. The x- and y-axes are logical, with the individuals categorized by treatment. Summary statistics, in this case medians, range, and IQR, are shown in in addition to the underlying data. Keeping the specific health parameter scores separate makes comparison easier for the viewer. 

c. The authors handled visual clutter very well. The plots are minimal, with no unnecessary grid lines or panels. They are effective in communicating a significant amount of information with minimal ink. 

d. Because there is blank space in the lower right corner of the figure, I would add a legend explaining what the treatment acronyms (WDC, DTC, NOC) stand for. This is explained in the figure caption, but I think it could be helpful to see right within the figure.  
